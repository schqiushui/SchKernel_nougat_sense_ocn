...
struct device *g_cyttsp5_dev = NULL;

//htc-


#define CY_CORE_STARTUP_RETRY_COUNT		3
#define CY_CORE_TOUCH_NORMAL_MODE		1
#define CY_CORE_TOUCH_GLOVE_MODE		3
...
static struct cyttsp5_core_data *g_cd;
#endif
...
void *htc_cyttsp5_get_dev(void)
{
	return (void*)g_cyttsp5_dev;
}

static bool htc_allow_retry_operation(struct cyttsp5_core_data *cd)
{
	if(cd && cd->cpdata) {
		if( atomic_read(&cd->cpdata->tp_power_state) == eTP_POWER_OFF_DONE ||
			atomic_read(&cd->cpdata->tp_power_state) == eTP_POWER_OFF_DOING ) {
			pr_err("%s: skip to retry, tp_power_state is %d",
				__func__,
				atomic_read(&cd->cpdata->tp_power_state));
			return false;
		}
	}
	return true;
...
/**
 * htc_cyttsp5_drv_pm_ops -
 *     for incell project:
 *     1. follow hw spec to know how to set touch driver suspend/resume
 *         - this need to cowork with display driver
 *     2. original linux driver dev_pm_ops and fb_notifier will be skiped
 */
void htc_cyttsp5_drv_pm_ops(int is_lcm_suspend)
{
	int rc = 0;
	struct device *dev = g_cyttsp5_dev;

	if(dev) {
		struct cyttsp5_core_data *cd = dev_get_drvdata(dev);

		pr_info("%s: is_lcm_suspend %d ++", __func__, is_lcm_suspend);

		if(cd && cd->mode != CY_MODE_OPERATIONAL) {
			pr_err("%s: mode %d, skip to do suspend and resume",
				__func__, cd->mode);
			return;
		}

		if(is_lcm_suspend == 0 && cd->cpdata) {
			atomic_set(&cd->cpdata->tp_power_state, eTP_POWER_ON_DOING);
			rc = cyttsp5_core_resume(dev);
			if(cd) {
				call_atten_cb(cd, CY_ATTEN_RESUME, 0);
			}
			atomic_set(&cd->cpdata->tp_power_state, eTP_POWER_ON_DONE);
		}
			...
			rc = cyttsp5_core_suspend(dev);
			atomic_set(&cd->cpdata->tp_power_state, eTP_POWER_OFF_DONE);
		}

		pr_info("%s: is_lcm_suspend %d --, rc %d", __func__, is_lcm_suspend, rc);

	}
}
EXPORT_SYMBOL(htc_cyttsp5_drv_pm_ops);

#ifdef CONFIG_NANOHUB_TP_SWITCH
void switch_sensor_hub(struct cyttsp5_core_data *cd, enum htc_touch_i2c_select select)
{
	int mask = 0;
	if( cd && cd->cpdata &&
		cd->cpdata->TP_I2C_SEL_CPU_gpio > 0 ) {

		if(cd->cpdata->tp_i2c_select == select) {
			pr_info("%s: tp_i2c_select already changed to %d, skip this", __func__, select);
			return;
		}

		if (cd->pdata->core_pdata->support_glove
				&& (cd->pdata->core_pdata->TP_mode == CY_CORE_TOUCH_GLOVE_MODE)) {
			mask = 0x01 << 1;
		}

		switch(select) {
			case eTP_I2C_SEL_CPU:
				pr_info("%s: TP_I2C_SEL_CPU_gpio to CPU\n", __func__);
				nanohub_tp_status(eTP_I2C_SEL_CPU);
				gpio_set_value(cd->cpdata->TP_I2C_SEL_CPU_gpio, eTP_I2C_SEL_CPU);
				cd->cpdata->tp_i2c_select = eTP_I2C_SEL_CPU;

				if (cd->irq_disabled) {
					pr_info("%s: enable_irq", __func__);
					enable_irq(cd->irq);
					cd->irq_disabled = 0;
				}

				break;
			case eTP_I2C_SEL_SHUB:

				pr_info("%s: disable_irq", __func__);
				disable_irq(cd->irq);
				cd->irq_disabled = 1;

				pr_info("%s: TP_I2C_SEL_CPU_gpio to SHUB\n", __func__);
				gpio_set_value(cd->cpdata->TP_I2C_SEL_CPU_gpio, eTP_I2C_SEL_SHUB);
				nanohub_tp_status(eTP_I2C_SEL_SHUB | mask);
				cd->cpdata->tp_i2c_select = eTP_I2C_SEL_SHUB;
				break;
			default:
				htc_dev_err(cd->dev, "%s: TP_I2C_SEL_CPU_gpio erro select %d\n", __func__, select);
				break;
		}
...
#endif
//htc++ support different touch mode
// 1: normal mode
// 3: glove mode
// others: disable mode
static int cyttsp5_set_status(struct device *dev, int value)
{
	int rc = 0;
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error on request exclusive r=%d\n", __func__, rc);
		goto exit;
	}

	rc = cyttsp5_request_nonhid_get_param(dev, 0,
		CY_RAM_ID_TOUCHMODE_ENABLED, &mode);
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error on request mode enable r=%d\n",
			__func__, rc);
		goto exit_release;
	}

	if (value == CY_CORE_TOUCH_NORMAL_MODE) {
		mode &= ~(1 << 1);
		mode |= 1;
		pdata->core_pdata->TP_mode = CY_CORE_TOUCH_NORMAL_MODE;
		pr_info("%s: Set touch mode as %d\n", __func__, value);
	} else if (value == CY_CORE_TOUCH_GLOVE_MODE) {
		mode |= 3;
		pdata->core_pdata->TP_mode = CY_CORE_TOUCH_GLOVE_MODE;
		pr_info("%s: Set touch mode as %d\n", __func__, value);
	} else {
		mode &= ~3;
		pdata->core_pdata->TP_mode = 0;
		pr_info("%s: Disable touch %d\n", __func__, value);
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	const char htc_cmd_mutual_cap_diff_data_id_2[] = "2";
	const char htc_cmd_self_cap_diff_data_id_5[] = "5";

	struct file* panel_scan_fd;
	struct file* cm_panel_fd;
	struct file* cp_panel_fd;
//	struct file* sysfs_fd;
...
//	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	...
	if( pdata &&
		pdata->core_pdata &&
		pdata->core_pdata->diag_cmd == eHTC_DIAG_CM_CP_PANEL ) {
		//cp_panel
		cp_panel_fd = filp_open(HTC_CYTTSP5_DEBUFFS_CP_PANEL, O_RDWR, 0);
		if (IS_ERR_OR_NULL(cp_panel_fd)) {
			pr_err("%s: open %s fail!!!\n", __func__, HTC_CYTTSP5_DEBUFFS_CP_PANEL);
		}
		else {
			ret = kernel_read(cp_panel_fd, 0, buf, PAGE_SIZE);
//			pr_info("%s: read %s, ret %d\n", __func__, HTC_CYTTSP5_DEBUFFS_CP_PANEL, ret);
			filp_close(cp_panel_fd, NULL);
		}
		//cm_panel
		cm_panel_fd = filp_open(HTC_CYTTSP5_DEBUFFS_CM_PANEL, O_RDWR, 0);
		if (IS_ERR_OR_NULL(cm_panel_fd)) {
			pr_err("%s: open %s fail!!!\n", __func__, HTC_CYTTSP5_DEBUFFS_CM_PANEL);
		}
		else {
			ret = kernel_read(cm_panel_fd, 0, buf, PAGE_SIZE);
//			pr_info("%s: read %s, ret %d\n", __func__, HTC_CYTTSP5_DEBUFFS_CM_PANEL, ret);
			filp_close(cm_panel_fd, NULL);
		}
	}
	else if( pdata &&
		pdata->core_pdata &&
		pdata->core_pdata->diag_cmd == eHTC_DIAG_PANEL_SCAN ) {
		panel_scan_fd = filp_open(HTC_CYTTSP5_DEBUFFS_PANEL_SCAN, O_RDWR, 0);
		if (IS_ERR_OR_NULL(panel_scan_fd)) {
			pr_err("%s: open %s fail!!!\n", __func__, HTC_CYTTSP5_DEBUFFS_PANEL_SCAN);
		}
		else {
			//panel_scan with id 5
			ret = kernel_write(panel_scan_fd,
					htc_cmd_self_cap_diff_data_id_5, sizeof(htc_cmd_self_cap_diff_data_id_5), 0);
			if(ret != sizeof(htc_cmd_self_cap_diff_data_id_5)) {
				pr_err("%s: kernel_write %s to panel_scan_fd fail, ret %d\n",
					__func__, htc_cmd_self_cap_diff_data_id_5, ret);
			}

			ret = kernel_read(panel_scan_fd, 0, buf, PAGE_SIZE);
//			pr_info("%s: read %s, ret %d\n", __func__, HTC_CYTTSP5_DEBUFFS_PANEL_SCAN, ret);

			//panel_scan with id 2
			ret = kernel_write(panel_scan_fd,
					htc_cmd_mutual_cap_diff_data_id_2, sizeof(htc_cmd_mutual_cap_diff_data_id_2), 0);
			if(ret != sizeof(htc_cmd_self_cap_diff_data_id_5)) {
				pr_err("%s: kernel_write %s to panel_scan_fd fail, ret %d\n",
					__func__, htc_cmd_mutual_cap_diff_data_id_2, ret);
			}

			ret = kernel_read(panel_scan_fd, 0, buf, PAGE_SIZE);
//			pr_info("%s: read %s, ret %d\n", __func__, HTC_CYTTSP5_DEBUFFS_PANEL_SCAN, ret);
			filp_close(panel_scan_fd, NULL);
		}
	}
	return ret;
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if( pdata &&
		pdata->core_pdata &&
		value == 2 ) {
		pr_info("%s: execute cm_panel and cp_panel command", __func__);
		pdata->core_pdata->diag_cmd = eHTC_DIAG_CM_CP_PANEL;
	}
	else if( pdata &&
		pdata->core_pdata &&
		value == 1 ) {
		pr_info("%s: execute panel_scan command", __func__);
		pdata->core_pdata->diag_cmd = eHTC_DIAG_PANEL_SCAN;
	}
	else {
...
static ssize_t cyttsp5_glove_setting_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_platform_data *pdata = dev_get_platdata(dev);
	size_t count = 0;

	count += snprintf(buf + count, PAGE_SIZE, "%d\n", pdata->core_pdata->TP_mode);

	return count;
}
static ssize_t cyttsp5_glove_setting_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct cyttsp5_platform_data *pdata = dev_get_platdata(dev);
	u8 value;

	if (!pdata->core_pdata->support_glove)
		return count;

	sscanf(buf, "%2hhx", &value);
	...
	pr_info("%s: glove_setting change to %2hhx\n", __func__, value);
	if (eTP_I2C_SEL_CPU == pdata->core_pdata->tp_i2c_select) {
		if (value == 1) {
			if (cyttsp5_set_status(dev, CY_CORE_TOUCH_GLOVE_MODE)) {
				pr_err("set glove mode error\n");
				return -EINVAL;
			}
		} else {
			if (cyttsp5_set_status(dev, CY_CORE_TOUCH_NORMAL_MODE)) {
				pr_err("set glove mode error\n");
				return -EINVAL;
			}
		}
		pr_info("%s: glove_status is %d\n", __func__, pdata->core_pdata->TP_mode);
	} else
...
static DEVICE_ATTR(glove_setting, (S_IWUSR|S_IRUGO),
	cyttsp5_glove_setting_show, cyttsp5_glove_setting_store);
//htc-
...
wait:
	...
	if (with_timeout) {
		...
		if (IS_TMO(t)) {
			htc_dev_err(cd->dev, "%s: tmo waiting exclusive access\n",
				__func__);
			return -ETIME;
		}
	} else {
		...
		struct cyttsp5_hid_cmd *hid_cmd)
{
	...
	rc = cyttsp5_adap_write_read_specific(cd, cmd_length, cmd,
			hid_cmd->read_buf);
	if (rc)
		htc_dev_err(cd->dev, "%s: Fail cyttsp5_adap_transfer %d\n", __func__,__LINE__);
		...
		struct cyttsp5_hid_cmd *hid_cmd)
{
	int rc;
	int t;
	u16 timeout_ms = 0; //htc: fix uninitialized warning
	int *cmd_state;
	...
	if (IS_TMO(t)) {
		htc_dev_err(cd->dev, "%s: HID output cmd execution timed out\n",
			__func__);
		rc = -ETIME;
		goto error;
	}
...
error:
	...
	*cmd_state = 0;
	mutex_unlock(&cd->system_lock);
	dev_info(cd->dev, "%s: rc %d, opcode %d, timeout_ms %d",
		__func__, rc, hid_cmd->opcode, timeout_ms); //htc
exit:
	return rc;
}
...
static int cyttsp5_hid_cmd_reset(struct cyttsp5_core_data *cd)
{
	...
	if (rc < 0) {
		htc_dev_err(cd->dev, "%s: fail get exclusive ex=%p own=%p\n",
	...
	if (release_exclusive(cd, cd->dev) < 0)
		htc_dev_err(cd->dev, "%s: fail to release exclusive\n", __func__);
...
//htc--
	...
	pr_info("%s: power_state %d ++", __func__, power_state); //htc

	rc =  cyttsp5_hid_exec_cmd_and_wait_(cd, &hid_cmd);
	if (rc) {
		htc_dev_err(cd->dev, "%s: Failed to set power to state: %d\n", //htc
				__func__, power_state);
//htc++
#ifdef CY_HID_SET_POWER_RETRY
		if(retry_cnt++ < CY_HID_SET_POWER_RETRY)
		{
			hid_cmd.timeout_ms = 100;
			pr_err("%s: timeout to retry, retry_cnt %d ", __func__, retry_cnt);
			goto retry_set_power;
		}
#endif
//htc--
	       return rc;
...
//htc++
		...
		{
			hid_cmd.timeout_ms = 100;
			pr_err("%s: validation failed to retry, retry_cnt %d, resp[2][3][4] %#x %#x %#x",
				__func__,
				retry_cnt,
				cd->response_buf[2],
				cd->response_buf[3],
			...
			if (rc < 0)
				htc_dev_err(cd->dev, "%s: HW reset failed r=%d\n",
				__func__, rc);
		}
#endif
//htc--
...
//htc+
	pr_info("%s: power_state %d --, rc %d, power_state(response) %d", __func__,
		power_state, rc, (cd->response_buf[3] & 0x3));
//htc-
		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	if (cd->response_buf[HID_OUTPUT_RESPONSE_REPORT_OFFSET]
		...
		htc_dev_err(cd->dev, "%s: HID output response, wrong report_id\n",
			__func__);
		return -EPROTO;
	}

	if (cd->response_buf[4] != HID_OUTPUT_BL_SOP) {
		htc_dev_err(cd->dev, "%s: HID output response, wrong SOP\n",
			__func__);
		return -EPROTO;
	}

	if (cd->response_buf[size - 1] != HID_OUTPUT_BL_EOP) {
		htc_dev_err(cd->dev, "%s: HID output response, wrong EOP\n",
			__func__);
		return -EPROTO;
	}
	...
	if (cd->response_buf[size - 3] != LOW_BYTE(crc)
		...
		htc_dev_err(cd->dev, "%s: HID output response, wrong CRC 0x%X\n",
			__func__, crc);
		return -EPROTO;
	}
	...
	if (status) {
		htc_dev_err(cd->dev, "%s: HID output response, ERROR:%d\n",
			__func__, status);
		return -EPROTO;
	}
		...
		struct cyttsp5_hid_output *hid_output, bool raw)
{
	...
	if (param_size > 4) {
		htc_dev_err(cd->dev, "%s: Invalid parameter size\n", __func__);
		return;
	}
	...
	if (cd->response_buf[2] != HID_APP_RESPONSE_REPORT_ID
		...
		htc_dev_err(cd->dev, "%s: Set Parameter command not successful\n",
			__func__);
		return;
	}
		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	if (rc)
		htc_dev_err(cd->dev, "%s: Fail cyttsp5_adap_transfer\n", __func__);
...
error:
	...
	cd->hid_cmd_state = 0;
	mutex_unlock(&cd->system_lock);
	dev_info(cd->dev, "%s: command_code = %d, timeout_ms = %d",__func__, hid_output->command_code, timeout_ms); //htc
exit:
	return rc;
}

//htc++
#ifdef CY_WATCHDOG_RETRY
...
retry_hid_output_null:

	rc = cyttsp5_hid_send_output_and_wait_(cd, &hid_output);
	if(rc) {
		pr_err("%s: error(%d) to retry, retry_cnt %d ",
			__func__, rc, retry_cnt);
		hid_output.timeout_ms = CY_HID_OUTPUT_TIMEOUT/2;
		if( retry_cnt++ < CY_WATCHDOG_RETRY &&
			htc_allow_retry_operation(cd) == true) {
			goto retry_hid_output_null;
		}
	}
...
static void cyttsp5_si_get_sensing_conf_data(struct cyttsp5_core_data *cd)
{
	...
	cd->panel_id = scd_dev->panel_id; //htc: vendor's WA to avoid launch app fail!
	scd->btn = scd_dev->btn;
...
static int cyttsp5_get_sysinfo_regs(struct cyttsp5_core_data *cd)
{
	...
	if (rc < 0) {
		dev_info(cd->dev, "%s: get btn data. rc = %d\n", __func__, rc); //htc
		return rc;
	}
		...
		struct cyttsp5_core_data *cd, u8 *panel_id)
{
	...
	if (rc == -EPROTO && cd->response_buf[5] == ERROR_COMMAND) {
		...
		*panel_id = PANEL_ID_NOT_ENABLED;
		...
		htc_dev_err(cd->dev, "%s: Error on Get Panel ID command\n",
			__func__);
		return rc;
	}
...
//htc++
#ifdef CY_HID_GET_HID_DESCRIPTOR_RETRY
	int retry_cnt = 0;
retry_get_hid_desc:
#endif
//htc--
	...
	if (rc) {
		htc_dev_err(dev, "%s: failed to get HID descriptor length and version, rc=%d\n",
			__func__, rc);
//htc++
#ifdef CY_HID_GET_HID_DESCRIPTOR_RETRY
		if( retry_cnt++ < CY_HID_GET_HID_DESCRIPTOR_RETRY &&
			htc_allow_retry_operation(cd) == true) {
			pr_err("%s: i2c failed to retry, retry_cnt %d", __func__, retry_cnt);
			goto retry_get_hid_desc;
		}
#endif
//htc--
		goto error;
	...
	if (IS_TMO(t)) {
		htc_dev_err(cd->dev, "%s: HID get descriptor timed out\n",
			__func__);
//htc++
#ifdef CY_HID_GET_HID_DESCRIPTOR_RETRY
		if(retry_cnt++ < CY_HID_GET_HID_DESCRIPTOR_RETRY &&
...
//htc--
	...
	dev_dbg(dev, "%s: HID len:%X HID ver:%X packet_id %d\n", __func__,
	...
	if (le16_to_cpu(desc->hid_desc_len) != sizeof(*desc) ||
		...
		htc_dev_err(dev, "%s: Unsupported HID version\n", __func__);
//htc++
#ifdef CY_HID_GET_HID_DESCRIPTOR_RETRY
		if( retry_cnt++ < CY_HID_GET_HID_DESCRIPTOR_RETRY &&
			htc_allow_retry_operation(cd) == true) {
			pr_err("%s: validation failed to retry, retry_cnt %d",
				__func__, retry_cnt);
			goto retry_get_hid_desc;
		}
#endif
...
static int cyttsp5_hw_soft_reset(struct cyttsp5_core_data *cd)
{
	...
	rc = cyttsp5_hid_cmd_reset_(cd);
	if (rc < 0) {
		htc_dev_err(cd->dev, "%s: FAILED to execute SOFT reset\n",
				__func__);
		return rc;
	}
...
{
//htc++
	if(htc_allow_retry_operation(cd) == false) {
		pr_err("%s: error operation mode ",__func__);
		return -EPERM;
	}
//htc--
	if (cd->cpdata->xres) {
	...
	htc_dev_err(cd->dev, "%s: FAILED to execute HARD reset\n", __func__);
	return -ENODEV;
}
...
continue_main_item:
	...
	if (buf != end) {
		htc_dev_err(cd->dev, "%s: Report descriptor length invalid\n",
			__func__);
		rc = -EINVAL;
		goto exit;
	}
	...
	if (collection_nest) {
		htc_dev_err(cd->dev, "%s: Unbalanced collection items (%d)\n",
...
static int cyttsp5_get_report_descriptor_(struct cyttsp5_core_data *cd)
{
	...
	if (rc) {
		htc_dev_err(cd->dev, "%s: Error parsing report descriptor r=%d\n",
			__func__, rc);
	}
...
//htc+
	if(cd->fb_state == FB_OFF) {
		pr_info("%s: return on sleep_state %d, fb_state %d", __func__, cd->sleep_state, cd->fb_state);
		return;
	}

	//WA to force to upgrade FW
	if(cd && cd->mode != CY_MODE_OPERATIONAL) {
		s64 ms_boottime = ktime_to_ms(ktime_get_boottime());

		//wait system partition ready
		if(ms_boottime > 10*1000) {
			if(cd->cpdata) {
				if(atomic_read(&cd->cpdata->loader_fw_upgrading)) {
					pr_info("%s: loader_fw_upgrading is already true, wait...",
						__func__);
					return;
				}
				else {
					rc = cyttsp5_get_hid_descriptor(cd, &cd->hid_desc);
					if (rc < 0) {
						htc_dev_err(cd->dev, "%s: Error on getting HID descriptor r=%d\n",
							__func__, rc);
					}
					else {
						cd->mode = cyttsp5_get_mode(cd, &cd->hid_desc);
						if(cd->mode != CY_MODE_OPERATIONAL) {
							pr_info("%s: trigger start_fw_upgrade() with force_fw_upgrade_from_core",
								__func__);
							cd->cpdata->force_fw_upgrade_from_core = 1;
		...
		mod_timer(&cd->watchdog_timer, jiffies +
				msecs_to_jiffies(CY_WATCHDOG_TIMEOUT*2));
		return;
	}
//htc-
...
queue_startup:
	if (rc) {
		htc_dev_err(cd->dev,
			"%s: failed to access device in watchdog timer rc %d startup_retry_count %d\n",
			__func__, rc, cd->startup_retry_count);
//htc++
...
#else
		if (cd->startup_retry_count > CY_WATCHDOG_RETRY_COUNT) {
			pr_err("%s: can't recover by cyttsp5_queue_startup() over %d times",
				__func__, CY_WATCHDOG_RETRY_COUNT);
		}

		pr_err("%s: watchdog failed, try to recover by cyttsp5_queue_startup()",
			__func__);
...
//	dev_dbg(cd->dev, "%s: Watchdog timer triggered\n", __func__); //htc
...
static int cyttsp5_core_poweroff_device_(struct cyttsp5_core_data *cd)
{
	...
	if (cd->irq_enabled) {
		pr_info("%s: disable_irq", __func__); //htc
	...
	if (rc < 0)
		htc_dev_err(cd->dev, "%s: HW Power down fails r=%d\n",
				__func__, rc);
	return rc;
}
...
//	dev_info(cd->dev, "%s:#0 size:%d\n", __func__, size); //htc

	/* check reset */
	if (size == 0) {
		dev_dbg(cd->dev,
			"%s: Reset complete, hid_reset_cmd_state %d, hid_cmd_state %d\n",
			__func__, cd->hid_reset_cmd_state, cd->hid_cmd_state);
	...
	if(report_id != HID_TOUCH_REPORT_ID &&
		report_id != HID_APP_RESPONSE_REPORT_ID &&
		report_id != HID_BL_RESPONSE_REPORT_ID
		) {
	...
	if (report_id != cd->sysinfo.desc.tch_report_id
		...
		is_command = 1;
//	dev_info(cd->dev, "%s:#2 is_command = %d \n", __func__, is_command); //htc
...
read:
	...
	if (rc) {
		htc_dev_err(dev, "%s: Error getting report, r=%d\n",
				__func__, rc);
		return rc;
	}
...
static int cyttsp5_reset(struct cyttsp5_core_data *cd)
{
	...
	if (rc < 0)
		htc_dev_err(cd->dev, "%s: %s dev='%s' r=%d\n", __func__,
...
static int cyttsp5_reset_and_wait(struct cyttsp5_core_data *cd)
{
	...
	if (IS_TMO(t)) {
		htc_dev_err(cd->dev, "%s: reset timed out\n",
			__func__);
		rc = -ETIME;
		goto error;
	}
...
static int _cyttsp5_request_reset(struct device *dev)
{
	...
	rc = cyttsp5_reset(cd);
	if (rc < 0) {
		htc_dev_err(cd->dev, "%s: Error on h/w reset r=%d\n",
			__func__, rc);
		mutex_lock(&cd->system_lock);
		cd->hid_reset_cmd_state = 0;
		mutex_unlock(&cd->system_lock);
...
struct cyttsp5_sysinfo *_cyttsp5_request_sysinfo(struct device *dev)
{
	...
	pr_err("%s: sysinfo is not ready", __func__); //htc
...
reset:
	...
	if (cd->mode == CY_MODE_BOOTLOADER) {
		...
		rc = cyttsp5_hid_output_bl_launch_app_(cd);
		if (rc < 0) {
			htc_dev_err(cd->dev, "%s: Error on launch app r=%d\n",
				__func__, rc);
			if (retry--)
				goto reset;
			goto exit;
...
static int cyttsp5_core_poweron_device_(struct cyttsp5_core_data *cd)
{
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: HW Power up fails r=%d\n", __func__, rc);
		goto exit;
	}

	if (!cd->irq_enabled) {
		pr_info("%s: enable_irq", __func__); //htc
		cd->irq_enabled = true;
		enable_irq(cd->irq);
	}
...
#else
	if (retry != CY_CORE_STARTUP_RETRY_COUNT) {
		if(htc_allow_retry_operation(cd) == true) {
			dev_dbg(cd->dev, "%s: Retry %d\n", __func__,
				CY_CORE_STARTUP_RETRY_COUNT - retry);
		}
		else {
			pr_err("%s: stop retry and return -EINVAL, due to lcm suspend!",
				__func__);
			return -EINVAL;
		}
	}
...
//htc--
...
#if 0 //vendor original codes
	dev_info(cd->dev, "%s: mode = %d \n", __func__, cd->mode); //htc
	/* Switch to bootloader mode to get Panel ID */
	if (cd->mode == CY_MODE_OPERATIONAL) {
		rc = cyttsp5_hid_output_start_bootloader_(cd);
		if (rc < 0) {
			htc_dev_err(cd->dev, "%s: Error on start bootloader r=%d\n",
				__func__, rc);
			if (retry--)
				goto reset;
			goto exit;
...
#else //htc: vendor's WA to avoid launch app fail!
	dev_dbg(cd->dev, "%s: Panel ID: 0x%02X mode %d \n",
		__func__, cd->panel_id, cd->mode);
	...
	if (cd->mode == CY_MODE_BOOTLOADER) {
		...
		dev_info(cd->dev, "%s: before cyttsp5_startup_() retry, mode %d \n",
			__func__, cd->mode); //htc
		...
		if (cd->mode == CY_MODE_BOOTLOADER) {
			if (retry--) {
				...
				pr_err("%s: device FW may be corrupted, "
					"need watchdog to trigger start_fw_upgrade()",
					__func__);
			}
			goto exit;
		}
	...
	if (cd->mode == CY_MODE_OPERATIONAL)
		...
		htc_dev_err(cd->dev, "%s: Unknown mode\n", __func__);
		rc = -ENODEV;
		mutex_unlock(&cd->system_lock);
		if (retry--)
			goto reset;
	...
	if (rc < 0) {
		htc_dev_err(cd->dev, "%s: Error on getting report descriptor r=%d\n",
			__func__, rc);
		if (retry--)
			goto reset;
		goto exit;
	...
	rc = cyttsp5_hid_output_get_sysinfo_(cd);
	if (rc) {
		htc_dev_err(cd->dev, "%s: Error on getting sysinfo r=%d\n",
			__func__, rc);
		if (retry--)
			goto reset;
		goto exit;
...
static void cyttsp5_startup_work_function(struct work_struct *work)
{
	...
	rc = cyttsp5_startup(cd, true);
	if (rc < 0)
		htc_dev_err(cd->dev, "%s: Fail queued startup r=%d\n",
			__func__, rc);
}
...
static int cyttsp5_core_rt_suspend(struct device *dev)
{
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error on sleep\n", __func__);
		return -EAGAIN;
	}
	return 0;
}
...
static int cyttsp5_core_rt_resume(struct device *dev)
{
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error on wake\n", __func__);
		return -EAGAIN;
	}
...
//htc+
#ifdef CONFIG_TOUCHSCREEN_MTK
int yttsp5_core_suspend(struct device *dev)
#else
static int cyttsp5_core_suspend(struct device *dev)
#endif
{

	enum cyttsp5_sleep_state current_sleep_state; //htc
	...
	pr_info("%s: sleep_state %d", __func__, cd->sleep_state);

	mutex_lock(&cd->system_lock);
	current_sleep_state = cd->sleep_state;
	mutex_unlock(&cd->system_lock);
	if( current_sleep_state == SS_SLEEP_ON ||
		current_sleep_state == SS_SLEEPING ) {
		pr_info("%s: already suspend, sleep_state %d, skip this", __func__, current_sleep_state);
		return 0;
	}
//htc-
...
{
//htc++
	enum cyttsp5_sleep_state current_sleep_state;
	int rc = 0;
//htc--
#ifdef CONFIG_TOUCHSCREEN_MTK
	struct cyttsp5_core_data *cd = g_cd;
	...
	current_sleep_state = cd->sleep_state;
	mutex_unlock(&cd->system_lock);
	if( current_sleep_state == SS_SLEEP_OFF ||
		current_sleep_state == SS_WAKING ) {
		pr_info("%s: already resume, sleep_state %d, skip this", __func__, current_sleep_state);
		return 0;
	}
//htc-
...
//htc++: turn off cyttsp5_pm_ops, incell panel will trigger suspend/resume
//       by display please check htc_cyttsp5_drv_pm_ops()
//#if HTC_FOLOW_CONFIG_FB_AND_PM_SLEEP_AND_PM_RUNTIME, OCN project need to change to 0
#if 0
//vendor original codes
...
//htc-
		...
		"Revision Control Number", cydata->revctrl, cydata->revctrl,
...
//htc+
		"hTC LCM id", (cd->cpdata)? cd->cpdata->htc_lcm_id : -1,
		"Panel Tx Number", sensing_conf_data->tx_num,
		"Panel Rx Number", sensing_conf_data->rx_num
//htc-
		);
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	rc = cyttsp5_startup(cd, true);
	if (rc < 0)
		htc_dev_err(dev, "%s: HW reset failed r=%d\n",
			__func__, rc);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	case CY_DBG_SUSPEND:
		...
		rc = cyttsp5_core_sleep(cd);
		if (rc)
			htc_dev_err(dev, "%s: Suspend failed rc=%d\n",
				__func__, rc);
		else
	...
	case CY_DBG_RESUME:
		...
		rc = cyttsp5_core_wake(cd);
		if (rc)
			htc_dev_err(dev, "%s: Resume failed rc=%d\n",
				__func__, rc);
		else
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(buf, CY_MAX_PRBUF_SIZE,
		...
		"TP_1V8_EN_gpio", pdata->core_pdata->TP_1V8_EN_gpio, //htc
		"TP_I2C_SEL_CPU_gpio", pdata->core_pdata->TP_I2C_SEL_CPU_gpio, //htc
...
//htc+
	__ATTR(vendor, S_IRUGO, cyttsp5_ic_ver_show, NULL),
	__ATTR(diag, S_IRUSR | S_IWUSR, diag_show, diag_store),
//htc-
};
...
static int add_sysfs_interfaces(struct device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(attributes); i++)
		if (device_create_file(dev, attributes + i))
			goto undo;
//htc+
	if(sysfs_create_link(NULL, &dev->kobj, "android_touch")) {
		htc_dev_err(dev, "%s: sysfs_create_link android_touch fail!!!\n", __func__);
	}
	if (pdata->core_pdata->support_glove) {
		if (sysfs_create_file(&dev->kobj, &dev_attr_glove_setting.attr) < 0) {
...
undo:
	...
	htc_dev_err(dev, "%s: failed to create sysfs interface\n", __func__);
	return -ENODEV;
}
...
static void cyttsp5_probe_modules(struct cyttsp5_core_data *cd)
{
	...
	list_for_each_entry(m, &module_list, node) {
		...
		if (rc)
			htc_dev_err(cd->dev, "%s: Probe fails for module %s\n",
				__func__, m->name);
	}
...
//htc++:
//1. use htc_cyttsp5_drv_pm_ops() call back func
//2. call cyttsp5_xres() for reset purpose
static void cyttsp5_resume_work_func(struct work_struct *work)
{
	struct cyttsp5_core_data *cd =
			container_of(work, struct cyttsp5_core_data,
					work_resume);
	int rc = 0;
#ifdef CY_HID_SET_MODE_RETRY
	int retry_cnt = 0;
#endif
...
#ifdef CY_HID_SET_MODE_RETRY
retry_set_mode:
	if (eTP_I2C_SEL_CPU == cd->pdata->core_pdata->tp_i2c_select && htc_allow_retry_operation(cd) == true) {
		if (cyttsp5_set_status(cd->dev, cd->pdata->core_pdata->TP_mode) != 0) {
			if (retry_cnt++ < CY_HID_SET_MODE_RETRY) {
				pr_err("%s: timeout to retry, retry_cnt %d ", __func__, retry_cnt);
				goto retry_set_mode;
			}
			pr_err("set touch mode error\n");
		} else
			pr_info("%s: touch mode change to %2hhx\n", __func__, cd->pdata->core_pdata->TP_mode);
	} else
		pr_info("%s: I2C already switch to MCU side\n", __func__);
#endif
	cd->fb_state = FB_ON;
...
#ifdef CONFIG_NANOHUB_TP_SWITCH
static void cyttsp5_switchHUB_work_func(struct work_struct *work)
{
	struct cyttsp5_core_data *cd =
			container_of(work, struct cyttsp5_core_data,
					work_switchHUB);
...
static void cyttsp5_switchCPU_work_func(struct work_struct *work)
{
	struct cyttsp5_core_data *cd =
			container_of(work, struct cyttsp5_core_data,
					work_switchCPU);
	switch_sensor_hub(cd, eTP_I2C_SEL_CPU);
}
#endif
//htc--
		...
		unsigned long event, void *data)
{
	...
	if ((event == FB_EARLY_EVENT_BLANK) && (*blank == FB_BLANK_UNBLANK)) {
...
#endif
			dev_info(cd->dev, "%s: UNBLANK! event %ld touch resume, call resume work\n", __func__, event);
		...
		if (cd->fb_state == FB_ON) {
			...
			dev_info(cd->dev, "%s: POWERDOWN! event %ld\n", __func__, event);
		 }
#ifdef CONFIG_NANOHUB_TP_SWITCH
	} else if ((event == FB_EVENT_BLANK) && (*blank == FB_BLANK_POWERDOWN)) {
		if (cd->fb_state == FB_ON) {
			dev_info(cd->dev, "%s: suspend end, call switch to sensor-hub\n", __func__);
...
static void cyttsp5_setup_fb_notifier(struct cyttsp5_core_data *cd)
{
	...
	if (rc)
		htc_dev_err(cd->dev, "Unable to register fb_notifier: %d\n", rc);
}
#endif
//htc--
#endif
...
static int cyttsp5_setup_irq_gpio(struct cyttsp5_core_data *cd)
{
...
#else
	/* Initialize IRQ */
	dev_info(dev, "%s: irq_gpio = 0x%x", __func__, cd->cpdata->irq_gpio); //htc
	...
	rc = request_threaded_irq(cd->irq, NULL, cyttsp5_irq, irq_flags,
		dev_name(dev), cd);
	if (rc < 0)
		htc_dev_err(dev, "%s: Error, could not request irq\n", __func__);
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	if (pdata->core_pdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP) {
		if (!pdata->core_pdata->power) {
			htc_dev_err(dev, "%s: Missing platform data function\n",
					__func__);
			rc = -ENODEV;
			goto error_no_pdata;
		}
	...
	if (!cd->I2CDMABuf_va) {
		htc_dev_err(dev, "%s Allocate DMA I2C Buffer failed!\n", __func__);
	...
	cd->resume_suspend_wq = create_singlethread_workqueue("resume_touch_work");
	if (!cd->resume_suspend_wq) {
		rc = -ENOMEM;
		goto error_resume_suspend_workqueue;
	}
	INIT_WORK(&cd->work_resume, cyttsp5_resume_work_func);
	INIT_WORK(&cd->work_suspend, cyttsp5_suspend_work_func);
#ifdef CONFIG_NANOHUB_TP_SWITCH
	INIT_WORK(&cd->work_switchHUB, cyttsp5_switchHUB_work_func);
	INIT_WORK(&cd->work_switchCPU, cyttsp5_switchCPU_work_func);
#endif
	...
	if (rc < 0) {
		htc_dev_err(cd->dev, "%s: HW Init fail r=%d\n", __func__, rc);
		goto error_init;
	}
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error, could not setup IRQ\n", __func__);
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error, fail sysfs init\n", __func__);
	...
	if (rc < 0)
		htc_dev_err(dev, "%s: Error, device_init_wakeup rc:%d\n",
				__func__, rc);
	...
	if (rc == -ENODEV) {
		htc_dev_err(cd->dev, "%s: Fail initial startup r=%d\n",
			__func__, rc);
		goto error_startup;
	}
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error, fail mt probe\n", __func__);
		goto error_startup;
	}

	rc = cyttsp5_btn_probe(dev);
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error, fail btn probe\n", __func__);
		goto error_startup_mt;
	}

	rc = cyttsp5_proximity_probe(dev);
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error, fail proximity probe\n", __func__);
...
error_resume_suspend_workqueue:
	if (cd->resume_suspend_wq) {
		destroy_workqueue(cd->resume_suspend_wq);
	}
#ifdef USE_I2C_DMA
